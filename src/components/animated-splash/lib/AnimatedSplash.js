...
import { View, Animated, StatusBar, StyleSheet,Text } from "react-native"
import styles, {
  _solidBackground,
  _dynamicLogoStyle,
        ...
        <View style={styles.containerGlue}>
          ...
          {!animationDone && (
            <View style={[StyleSheet.absoluteFill, styles.logoStyle]}>
              <Text style={{ color: '#FFF', fontSize: 36 }}>{title}</Text>
               <Text style={{ color: '#FFF', fontSize: 28 }}>{subTitle}</Text>
              {customComponent ? (
                <Animated.View
